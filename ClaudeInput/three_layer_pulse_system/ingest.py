...
# ═══════════════════════════════════════════════════════════
# DATABASE (same schema as curriculum_pulse.py)
# ═══════════════════════════════════════════════════════════
...
class KnowledgeDB:
    ...
    def _create_tables(self):
        self.cursor.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS confusion_map (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                subject TEXT, confused_with TEXT, reason TEXT,
    ...
    def add_queue_task(self, task_type, payload, priority=5):
        self.cursor.execute(
            ...
            (task_type, json.dumps(payload) if isinstance(payload,dict) else payload, priority))
        self.conn.commit()
        return self.cursor.lastrowid
    
    def get_next_task(self):
        self.cursor.execute(
            "SELECT * FROM agent_queue WHERE status='pending' ORDER BY priority DESC, id ASC LIMIT 1")
        row = self.cursor.fetchone()
        if row:
            self.cursor.execute("UPDATE agent_queue SET status='processing' WHERE id=?", (row['id'],))
            self.conn.commit()
            return dict(row)
        return None
    ...
    def complete_task(self, task_id, result=""):
        self.cursor.execute(
            "UPDATE agent_queue SET status='done', result=?, completed_at=CURRENT_TIMESTAMP WHERE id=?",
            (result, task_id))
        self.conn.commit()
...
class OllamaLLM:
    ...
    def decompose_text(self, text, context=""):
        """Core operation: turn prose into atomic facts."""
